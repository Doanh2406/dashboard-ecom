...
export default function TableHeader({ home, count, setCount, sort, handleSort, title }) {
  const [actionBig, setActionBig] = useState(false)
  const [search, setSearch] = useState('')
  const dispatch = useDispatch()
  const { loading, error, product } = useSelector(state => state.productSearch)
  useEffect(() => {
  ...
  return (
    <div className='tb_header'>
      ...
      </select>
      {
        ...
        </select> : <select className='tb_select'>
          <option>10</option>
          <option>20</option>
          <option>30</option>
      ...
      <div>
        <form class="search-container">
          ...
          {
          product && <div  className='search_container_form'>
            {
              product.map(item => {
                return (
                  <NavLink to={`/shopping/${item._id}`} style={{ textDecoration: 'none' }}>
                    <div className='search_container_item' >

                      <img src={'http://localhost:5000/upload/product/' + item.image[0].filename} />
